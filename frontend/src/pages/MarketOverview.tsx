  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden p-3 gap-3">
        {/* Stats strip */}
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-2 flex-shrink-0">
          <StatCard label="Tracked Pairs"    value={totalPairs}                     sub="All USDT Perps" />
          <StatCard label="Arb Opportunities" value={arbitrageOpportunities.length}  sub="Cross-exchange spread" color="#f59e0b" />
          <StatCard label="Active Signals"    value={signals.length}                 sub="Crowd positioning" color="#8b5cf6" />
          <StatCard label="Extreme Funding"   value={extreme}                        sub="Abs rate > 0.05%" color="#ef4444" />
          <StatCard label="Highest Long" value={topPos ? `${topPos.avgRate.toFixed(3)}%` : '—'} sub={topPos?.symbol.replace('USDT','')} color="#ef4444" />
          <StatCard label="Highest Short" value={topNeg ? `${topNeg.avgRate.toFixed(3)}%` : '—'} sub={topNeg?.symbol.replace('USDT','')} color="#10b981" />
        </div>
        ...
        <div className="flex-1 grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-3 min-h-0">
          ...
          {/* Left: Table */}
          <div className="bg-[#0d1017] border border-[#1e2430] rounded-lg overflow-hidden flex flex-col min-h-0">
            <FundingTable />
          </div>

          {/* Right: Heatmap + Arb + Signals */}
          <div className="flex flex-col gap-3 min-h-0">
            {/* Funding Heatmap */}
            <div className="bg-[#0d1017] border border-[#1e2430] rounded-lg flex-shrink-0">
            ...
            {volumeSpikes.length > 0 && (
              <div className="bg-[#0d1017] border border-[#1e2430] rounded-lg flex-shrink-0">
                ...
                <div className="p-2 flex flex-col gap-1 max-h-36 overflow-y-auto">
                  {volumeSpikes.slice(0,8).map(v => (
                    <div key={v.symbol} className="flex items-center justify-between px-2 py-1 rounded bg-[#0a0c10] hover:bg-[#161c26]">
                      ...
                      <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase ${
                        v.type === 'BUYING' ? 'bg-[#10b981]/10 text-[#10b981]' : 'bg-[#ef4444]/10 text-[#ef4444]'
                      }`}>{v.type}</span>
                    </div>
                  ))}
                </div>
